...
指摘メッセージは、日本語 v1 Check の `CheckMessage.message` をそのまま利用し、Presentation 側で個別ルールの判定理由を再構成しない。原文条件を伴う Warning では、原文と翻訳の比較を利用者の判断材料とする。
...
- 同じ翻訳文字列で同じルールに複数の原因がある場合も、v1 では位置情報や該当箇所一覧を結果契約へ追加しない。必要な表記ごとに独立した `CheckMessage` を返してよい。
- 別の原因または別のルールに該当する場合は、同じ翻訳文字列であっても別の指摘として表示してよい。

### 指摘対象の特定

v1 の日本語チェック結果は `entryIndex`、Severity ごとの `CheckMessage`、`styleGuideItem`、`message` を最小契約とする。

問題位置の範囲情報、専用の判定理由、Finding Coordination は結果契約へ含めない。利用者は原文・翻訳の比較、指摘メッセージ、スタイルガイド項目から対象内容を確認する。
...
- 後のスペースが1つでない: `「:」の後に半角スペースを入れてください`
...
| FR-06 | 各指摘から Severity、指摘メッセージ、原文、翻訳を確認し、内容を判断できる。                                                      |
