...
v1 では、誤検出を避けるため、文字自体から日本語の句読点として不適切であることを高い確度で判断できる `，` `．` `､` `｡` を検出対象とする。
...
- 1-1 の検出対象である `，` `．` `､` `｡` は 1-1 で扱う。特に `，` `．` を、1-2 の全角記号から半角記号への変換対象として `,` `.` に案内しない
- 丸括弧の全角・半角は 1-5 で扱う
- 「」や『』など、日本語表記として使用する記号
...
- 1-1 の検出対象である `，` `．` `､` `｡` は 1-2 ではなく 1-1 として指摘する。
