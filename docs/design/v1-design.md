...
- v1 では、確認用の修正案を画面内で一時入力して再チェックできる。一方、`.po` ファイルそのものの翻訳編集・保存・修正版生成は行わない。
- 意味の違いを色だけで伝えない。
...
確認が正常に完了した場合、結果概要の中で WTC のチェック範囲を確認できるようにする。

案内は結果概要の Error / Warning 件数より強く目立たせず、結果概要の中で常時表示する。利用者が開閉操作をしなくても、WTC の確認範囲と手動確認が必要な領域を把握できることを優先する。

見出しでは「WTC のチェック範囲」と、現在の v1 で13項目をチェックしていることを短く示す。

案内では以下を示す。

- WTC はスタイルガイド全体ではなく、機械的に判定できる項目を確認すること
- 「自動チェック」「一部チェック」「手動確認」の意味
- 自然な日本語、訳語・ボタン名の統一、カタカナ語・長音表記、ブランド名・機能名、日付・日時、プレースホルダーなど、手動で確認したい主要項目
- README の利用者向け対応状況一覧、要件定義書、WordPress 日本語翻訳スタイルガイドへの導線

「未対応」という不足の表示ではなく、翻訳作業における役割として「手動確認」と表現する。ただし README の公開サマリーでは、要件定義書における v1 対象外および WordPress 共通チェックの分類と矛盾しないようにする。
...
1件以上の指摘がある場合、ルールフィルター適用後の指摘をページ単位に分けて表示する。

1ページあたりの表示件数は25件、50件、100件から選択でき、初期値は50件とする。表示件数を変更した場合は1ページ目へ戻る。

ページ移動では、前へ・次へに加えてページ番号を表示する。総ページ数が多い場合は、先頭3ページ、現在ページの前後1ページ、末尾3ページを基本として表示し、連続しない範囲は「…」で省略する。総ページ数が少なく各範囲が連続する場合は省略記号を表示せず、全ページ番号を表示する。

ページ番号、前へ・次へ、省略記号は1つながりの操作として表示し、外周を共有する。現在ページは主要操作色のブルー背景と白文字で示し、同時に `aria-current="page"` を付与して色だけに依存しない。

ページ番号は直接入力でも指定できる。入力途中では移動せず、Enter またはフォーカスアウトで確定する。1から総ページ数までの整数だけを有効とし、無効な値では現在ページを変更しない。

直接入力欄は指摘一覧の上部・下部それぞれが編集中の文字列を個別に保持する。確定した現在ページと表示件数は上下で共有し、一方の操作でページが変わった場合、もう一方の非編集中入力欄は現在ページへ追従する。

現在ページでは「801–850 / 1217件の指摘」のように、表示中の範囲とフィルター後の総件数を示す。最終ページは実際に残っている件数までを表示する。

ページネーション操作は指摘一覧の上部と下部に同じ内容を配置する。上下は同じ現在ページと表示件数を共有し、どちらから操作しても同じ表示範囲へ反映する。総ページ数が1ページの場合は不要なページ番号・前へ・次へを表示せず、表示件数の選択は利用できる状態を保ってよい。

ルールフィルターを変更した場合、別のファイルを選択した場合、新しい確認を開始した場合は1ページ目へ戻る。ページ移動や表示件数変更だけでファイル確認を再実行しない。

ページネーションは指摘一覧の画面表示だけに作用する。結果概要の Error / Warning 件数と保存・コピー対象は正常完了した確認結果全体を維持する。
...
各指摘では、現在表示している翻訳をもとに、修正案をその場で入力して再チェックできるようにする。

「修正して再チェック」を選ぶと、その指摘の翻訳を初期値とした入力欄を同じ指摘内に表示する。利用者は修正案を入力し、「再チェック」でその修正案だけを確認できる。「キャンセル」では修正案を破棄し、通常の指摘表示へ戻る。

再チェックで問題が検出されなかった場合は、「この翻訳では問題は見つかりませんでした。」と表示する。問題が残っている場合は、Error / Warning、指摘メッセージ、問題箇所、スタイルガイド項目、一次情報への導線を再チェック結果として表示する。再チェック後に修正案をさらに変更した場合は、以前の再チェック結果を現在の修正案の結果として残さず、再度チェックするまで結果を表示しない。

この入力は、修正内容が WTC の対象ルールを満たすか確認するための一時的な作業領域とする。修正案は元の `.po` ファイル、ファイル全体の Error / Warning 件数、ルールフィルター、ページネーション、CSV / JSON / Markdown の保存・コピー内容へ反映しない。

WTC 上で `.po` ファイルの翻訳内容そのものを編集・保存する機能とは区別する。修正案の入力内容をファイルへ適用したり、修正版ファイルを生成したりしない。
...
- ファイルの確認が正常に完了したこと
- WTC の自動チェックでは問題が検出されなかったこと
- 手動で確認したい項目が残っていること

翻訳全体が完全に正しいことや、WordPress 日本語翻訳スタイルガイドのすべてを満たしていることを保証する表現にはしない。チェック範囲の案内から、利用者が手動確認すべき領域を続けて確認できるようにする。
...
翻訳本文に全角中点 `・` または半角中黒 `･` が含まれる場合、ルール 5 の確認対象として Warning とする。

スタイルガイドでは中点を原則使用しない一方、カタカナが続いて区切りが分かりにくい場合などに使用できる例外があるため、中点の存在だけで Error とはしない。

同一翻訳内に複数の中点がある場合は、1件の Warning にまとめて全箇所の位置を保持する。

**指摘しない場合**

URL、コード表記、マークアップなど、既存の技術文字列保護対象の内部だけに中点が存在する場合は指摘しない。
...
| FR-13 | フィルター後の指摘を25 / 50 / 100件単位でページ分割し、上下の同一ページ操作、ページ番号、直接入力、表示範囲を提供する。ページネーションは結果概要と保存・コピー対象を変更しない。 |
