...
日本語本文の読点・句点として `,` `.` `，` `．` `､` `｡` が使われている場合に指摘し、読点は「、」、句点は「。」を期待する表記として示す。

これらの文字であっても、日本語本文の句読点ではなく数値や技術的な文字列などの一部として使われている場合は、このルールでは指摘しない。
...
問題となる句読点を強調し、読点は「、」、句点は「。」が期待する表記であることを示す。
...
- 日本語本文の読点・句点として使われている `,` `.` `，` `．` `､` `｡` は 1-1 で扱う。特に `，` `．` を、1-2 の全角記号から半角記号への変換対象として `,` `.` に案内しない
- 丸括弧の全角・半角は 1-5 で扱う
- 「」や『』など、日本語表記として使用する記号
...
- 日本語本文の読点・句点として使われている `,` `.` `，` `．` `､` `｡` は 1-2 ではなく 1-1 として指摘する。
