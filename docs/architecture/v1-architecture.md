...
フィルター後の指摘一覧について、現在ページと表示件数から総ページ数、現在範囲、現在ページの指摘、先頭・現在周辺・末尾からなるページ番号表示を導出する。確定済みの現在ページと表示件数は Result Presentation が共有状態として所有し、上下のページ操作は同じ状態を参照する。ページ直接入力の確定前文字列は各操作位置のローカルな編集状態とし、確認結果へ含めない。
...
- ルールフィルターは既存の指摘に付与されたスタイルガイド項目だけを利用し、Validation Core / Check Orchestration の公開契約を変更しない。
- 結果概要の Error / Warning 件数は正常完了結果全体から導出し、ルールフィルターやページネーションでは変更しない。
- ページネーションはルールフィルター適用後の指摘にだけ作用し、元の確認結果を分割・変更しない。
- ページ数、表示範囲、現在ページの指摘、ページ番号表示はフィルター後の指摘と現在ページ・表示件数から導出し、重複した永続状態として保持しない。
...
フィルターやページネーションは Presentation 内の画面表示だけに作用する。ルールフィルターでは、現在の正常完了結果に含まれる指摘から選択肢・ルール別件数・表示対象を導出し、元の確認結果を変更しない。

ページネーションはフィルター後の指摘を入力とし、現在ページ・表示件数から画面に表示する部分集合を導出する。初期表示件数は50件とし、25 / 50 / 100件から選択する。ページ状態は Validation Core / Check Orchestration の契約へ持ち込まず、専用 store、service、virtualization layer を設けない。

結果概要の Error / Warning 件数は現在の正常完了結果全体を表し、フィルター後・ページネーション後の表示件数とは分離する。結果の保存・コピーも、表示中の部分集合ではなく現在の正常完了結果全体を対象とする。
